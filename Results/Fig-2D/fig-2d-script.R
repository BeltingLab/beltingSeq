...
# Create Fig-2D: Enrichment plot of selected gene sets in CCLD, U3054MG, and U87MG cells
# Define paths
output_dir <- file.path(wd, "Results", "Fig-2D")
...
FIG2D.enrichplot <- list()
FIG2D.enrichplot$GBM$scores  <- do.call(rbind, c(
  lapply(names(tgfb.ranks)[1:2], function(i){
    gsInfo(tgfb.ranks[[i]], object = GSEA.object[[i]]) %>% 
      dplyr::mutate(source = i)}
  )))

FIG2D.enrichplot$GBM$scores <- FIG2D.enrichplot$GBM$scores %>% 
  dplyr::mutate(
    Description = gsub(x = FIG2D.enrichplot$GBM$scores$Description,
                       pattern = "KEGG_", replacement =  ""),
    source = factor(source, levels = names(GSEA.object)[1:2]))

FIG2D.enrichplot$GBM$table <- getEnrichmentTable(
  .df = rbind(CCLD.GSEA$df[tgfb.ranks$CCLD,] %>% 
...
(FIG2D.enrichplot$GBM$plot  <- cowplot::plot_grid(
  plotRunningScore(.df = FIG2D.enrichplot$GBM$scores, 
                   .x = "x", .y = "runningScore", 
                   .color = "source", .palette = c("U3054" = "darkred", 
                                                   "CCLD" = "steelblue"
                   )),
  plotGeneRank(.df = FIG2D.enrichplot$GBM$scores, 
               .x = "x", .facet = "source~.",
               .color = "source", .palette = c("U3054" = "darkred",
                                               "CCLD" = "steelblue"
               )),
...
FIG2D.enrichplot$U87$scores <- do.call(rbind, c(
  lapply(names(tgfb.ranks)[3:4], function(i){
    gsInfo(tgfb.ranks[[i]], object = GSEA.object[[i]]) %>% 
      dplyr::mutate(source = i)}
  )))
FIG2D.enrichplot$U87$scores <- FIG2D.enrichplot$U87$scores %>% 
  dplyr::mutate(
    Description = gsub(x = FIG2D.enrichplot$U87$scores$Description,
                       pattern = "KEGG_", replacement =  ""),
    source = factor(source, levels = names(GSEA.object)[c(3,4)]))

FIG2D.enrichplot$U87$table <- getEnrichmentTable(
  .df = rbind(U87.GSEA$`acu_pH64-control_acu`$df[tgfb.ranks$U87_acu,] %>% 
...
(FIG2D.enrichplot$U87$plot <- cowplot::plot_grid(
  plotRunningScore(.df = FIG2D.enrichplot$U87$scores, 
                   .x = "x", .y = "runningScore", 
                   .color = "source", .palette = c("U87_sel" = "#F00000",
                                                   "U87_acu" = "salmon")),
  plotGeneRank(.df = FIG2D.enrichplot$U87$scores, 
               .x = "x", .facet = "source~.",
               .color = "source", .palette = c("U87_sel" = "#F00000",
                                               "U87_acu" = "salmon")),
  byrow = T, nrow = 2, ncol = 1, scale = .95, 
...
(FIG2D.enrichplot$TOTAL_plot <- cowplot::plot_grid(
  FIG2D.enrichplot$GBM$plot,
  FIG2D.enrichplot$U87$plot,
  byrow = T, nrow = 2, ncol = 1, scale = .95,
  margins = c(0.5, 0.5, 0.5, 0.5)
))
...
# Save results table
openxlsx::write.xlsx(list(FIG2D.enrichplot$GBM$table,FIG2D.enrichplot$U87$table), 
                     sheetName = c("Upper panel", "Lower panel"), rowNames = TRUE,
                     file.path(output_dir, "Fig-2D-table.xlsx"))
...
ggsave(file.path(output_dir, "Figures", "Fig-2D-enrichplot-vector.svg"),
       device = "svg", plot = FIG2D.enrichplot$TOTAL_plot,
       bg = "white", width = 6, height = 9, units = "in")
# Save as png for presentation
ggsave(file.path(output_dir, "Figures", "Fig-2D-enrichplot-print.png"),
       device = "png", plot = FIG2D.enrichplot$TOTAL_plot,
